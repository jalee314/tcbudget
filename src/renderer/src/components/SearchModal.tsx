...
export default function SearchModal({ isOpen, onClose, onAddCard, sealedItems = [] }: SearchModalProps) {
  const [activeTab, setActiveTab] = useState<'cards' | 'sealed'>('sealed')
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-200">
          <div>
            ...
            <p className="text-xs text-surface-500 mt-0.5">Search cards & sealed products</p>
          </div>
        ...
        <div className="flex border-b border-surface-200 px-6">
          {(['sealed', 'cards'] as const).map((tab, i) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); handleReset() }}
              className={`py-3 ${i < 1 ? 'mr-6' : ''} text-sm font-semibold border-b-2 transition-colors ${activeTab === tab ? 'border-accent text-surface-900' : 'border-transparent text-surface-500 hover:text-surface-700'}`}
            >
              {tab === 'sealed' ? 'Sealed Products' : 'Single Cards'}
            </button>
          ))}
        </div>
